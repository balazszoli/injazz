...
articles = (
    {'InJazz': '101587', 'EU': 'YA-0020'}, {'InJazz': '101588', 'EU': 'YA-0021'}, {'InJazz': '101589', 'EU': 'YA-0002'},
    ...
    {'InJazz': '102015', 'EU': 'TW130ASMOM'}, {'InJazz': '102018', 'EU': 'TW138ASMDCE'},
    {'InJazz': '102019', 'EU': 'TW138ASMSD'},
...
ftp_message = ""
MB_title = "Import data from InJazz"

id_mb = ctypes.windll.user32.MessageBoxW(0, "Do you want to download new datafile from FTP? ", MB_title, 0x04)
if id_mb == 6:  # OK pressed
    # retrieve file from ftp
    ftp = FTP('91.235.69.81')
    ftp.login('user45', 'password')
    file_name = 'NEW.csv'
    ...
    ftp_message = "Datafile downloaded! "

ftp_message += "Processing started... \nYou will get a notification when file for import is ready."
ctypes.windll.user32.MessageBoxW(0, ftp_message, MB_title, 0)
...
with open('NEW.csv', encoding='utf-8') as csv_file:  # file from Injazz with data to change for import
    ...
    with open("injazz2import.csv", "w", encoding='utf-8', newline='') as import_file:
        ...
        for row in csv_reader:
            if (len(row) > 5) and (row[1] != '') and (row[5] != '') and (float(row[5].replace(',', '.')) > 3.0):
    ...
    #             print(f'█', end="")
    # print('| - Complete!')
ctypes.windll.user32.MessageBoxW(0, "File <injazz2import.csv> for importing to Horoshop is ready!", MB_title, 0)
